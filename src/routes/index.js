...
router.get("/logout", (ctx) => {
  const username = ctx.session.username || "Unknown User";
  ctx.session = null;
    ...
    try {
      ...
      await discordService.sendNotification({
        ...
        fields: [
          ...
          {
            name: "Cleared By",
            value: ctx.session.username || "Unknown User",
            inline: true,
          },
        ],
      });
...
router.post("/api/apps/:appName/reload", isAuthenticated, async (ctx) => {
  const { appName } = ctx.params;
  const initiatedBy = ctx.session.username || "Unknown User";
  try {
    ...
    if (Array.isArray(apps) && apps.length > 0) {
      ...
      await discordService.notifyAppActionComplete(
        appName,
        "reload",
        initiatedBy
      );
    ...
    await discordService.notifyAppActionFailed(
      appName,
      "reload",
      initiatedBy,
      err
...
router.post("/api/restart-server", isAuthenticated, async (ctx) => {
  ...
  try {
    ...
    ctx.body = {
      ...
      message:
        error.message ||
        "Failed to restart server. Please check if you have sudo permissions.",
    };
  }
});
...
router.post("/api/restart-mariadb", isAuthenticated, async (ctx) => {
  ...
  try {
    ...
    ctx.body = {
      ...
      message:
        error.message ||
        "Failed to restart MariaDB. Please check if you have sudo permissions.",
    };
  }
});
